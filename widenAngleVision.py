...
## Messages
messages =  {'title_msg': Message(gameDisplay, "Widen your angle vision", 'freesansbold.ttf', 30,
                          (display_width/2, display_height//8), color['blue'], white),
             'instruction_msg': Message(gameDisplay, "Focus your eyes on the green circle and type the popping-up symbols", 'freesansbold.ttf', 20,
                          (display_width/2, display_height//4), color['blue'], white),
             'pick_level_msg': Message(gameDisplay, "Pick your training level", 'freesansbold.ttf', 25,
                                      (display_width//2, display_height//2 - 70), color['blue'], white)}
...
def game_intro():
    ...
    run_btn = Button(150, 700, 100, 50, color['blue'], color['light_blue'],"Run", game_loop)
    quit_btn = Button(550, 700, 100, 50, color['red'], color['bright_red'], "Quit", quit_game)
    
    pick_level_scale = Scale(gameDisplay, 100, display_height/2 - 25, 600, 40, 1, 1, 3, 1,
...
def game_pause():
    ...
    continue_btn = Button(150, 700, 100, 50, color['blue'], color['light_blue'],"Continue", game_loop)
    quit_btn = Button(550, 700, 100, 50, color['red'], color['bright_red'], "Quit", quit_game)
    pick_level_scale = Scale(gameDisplay, 100, display_height/2 - 25, 600, 40, 1, 1, 3, 1,
                        color['red'], black, color['yellow'], color['blue'])
...
def get_background(width, height):
    ...
    circ = pygame.draw.circle(symbol_surface, color['green'], display_center, 45)
...
def game_loop():
    ...
    global total, correct, wrong
    global time_pause
    pause_btn = Button(325, 700, 100, 50, color['blue'], color['light_blue'],"Pause", game_pause)
